...
Using CPU staging hook for non-CUDA-aware MPI
"""
...
def cpu_staging_allreduce_hook(state, bucket):
    ...
    print(f"[HOOK-{rank}] Bucket allreduce started, size: {tensor.shape}", flush=True)
    sys.stderr.flush()
    ...
    print(f"[HOOK-{rank}] Bucket allreduce completed", flush=True)
    sys.stderr.flush()
...
def sync_buffers_on_cpu(model, rank):
    """
    Manually sync all model buffers on CPU (for non-CUDA-aware MPI).
    Alternative to DDP's automatic buffer broadcast - only if needed.
    """
    # For now, skip manual buffer sync
    # DDP with broadcast_buffers=True will handle it
...
def train_epoch(model, train_loader, criterion, optimizer, epoch, rank, device):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # Backward - hook fires here and syncs gradients via MPI
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def main():
    ...
    # Wrap with DDP on CPU (WITHOUT buffer broadcasting - only gradient sync via hook)
    model = DDP(
        model, 
    ...
    # Sync buffers ONCE at start (replaces DDP's init_sync)
    # sync_buffers_on_cpu(model.module, rank)  # DDP handles this now
    dist.barrier()
    
    if rank == 0:
        print(f"[Setup] DDP will sync buffers automatically with broadcast_buffers=True")
    
    # Register CPU staging hook for gradient synchronization ONLY
    model.register_comm_hook(state=None, hook=cpu_staging_allreduce_hook)
    
    # Ensure DDP syncs gradients during backward
    model.require_backward_grad_sync = True
    
    if rank == 0:
        print(f"[Setup] CPU staging hook registered (gradients only)\n")
