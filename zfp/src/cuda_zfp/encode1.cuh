...
//NEW: modified so it can accept a stream parameter
template<class Scalar>
                     ...
                     cudaStream_t cuda_stream)
{
  ...
  cudaMemsetAsync(stream, 0, stream_bytes, cuda_stream);
  ...
  cudaEncode1<Scalar> <<<grid_size, block_size, 0, cuda_stream>>>
    (maxbits,
     d_data,
     stream,
     dim,
...
size_t encode1(int dim,
               ...
               cudaStream_t cuda_stream)
{
  return encode1launch<Scalar>(dim, sx, d_data, stream, maxbits, cuda_stream);
}
...
size_t encode1(int dim,
               ...
               const int maxbits)
{
  return encode1launch<Scalar>(dim, sx, d_data, stream, maxbits, 0);
}
