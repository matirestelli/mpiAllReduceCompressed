...
//
// NEW: stream-aware 1D encode path.
//
// This is used by the AllReduce compression hooks, where each DDP gradient
// bucket is already flattened to a 1D CUDA tensor.  We intentionally support
// only the 1D path here because that is the only path needed by the framework.
// The original encode(...) function above remains unchanged for compatibility.
//
template<typename T>
size_t encode_stream(uint dims[3],
                     int3 stride,
                     int bits_per_block,
                     T *d_data,
                     Word *d_stream,
                     cudaStream_t cuda_stream)
{
  ...
  if(d == 1)
  {
    int dim = dims[0];
    int sx = stride.x;
    stream_size = cuZFP::encode1<T>(
        dim, sx, d_data, d_stream, bits_per_block, cuda_stream);
  }
  else
  {
    std::cerr << "stream-aware cuZFP wrapper currently supports only 1D fields\n";
  }

  errors.chk("Encode stream");
  return stream_size;
}
...
//
// NEW: stream-aware 1D decode path.
//
// This mirrors encode_stream(...).  The decompression kernel is launched on the
// caller-provided CUDA stream so the AllReduce hook can place decompression and
// the following reduction on the same non-default stream.
//
template<typename T>
size_t decode_stream(uint ndims[3],
                     int3 stride,
                     int bits_per_block,
                     Word *stream,
                     T *out,
                     cudaStream_t cuda_stream)
{
  ...
  if(d == 1)
  {
    uint dim = ndims[0];
    int sx = stride.x;
    stream_bytes = cuZFP::decode1<T>(
        dim, sx, stream, out, bits_per_block, cuda_stream);
  }
  else
  {
    std::cerr << "stream-aware cuZFP wrapper currently supports only 1D fields\n";
...
//
// NEW: exported stream-aware compression entry point.
//
// This function is parallel to cuda_compress(...), but calls encode_stream(...)
// so the actual cuZFP kernel launches on cuda_stream instead of the default
// stream.
//
// NOTE for the AllReduce framework:
// setup_device_stream_compress/decompress and
// setup_device_field_compress/decompress are safe for our use case because the
// PyTorch tensors are already CUDA tensors.  In that case these helpers return
// the existing device pointers and avoid host/device copies.  The stream-aware
// path should not be used with host tensors unless the host-copy helper logic is
// also made stream-safe.
//
size_t
cuda_compress_stream(zfp_stream *stream,
                     ...
                     cudaStream_t cuda_stream)
{
  ...
  {
    float *data = (float*) d_data;
    stream_bytes = internal::encode_stream<float>(
        dims, stride, (int)stream->maxbits, data, d_stream, cuda_stream);
  }
  else if(field->type == zfp_type_double)
  {
    double *data = (double*) d_data;
    stream_bytes = internal::encode_stream<double>(
        dims, stride, (int)stream->maxbits, data, d_stream, cuda_stream);
  }
  else
  {
    std::cerr << "stream-aware cuda_compress_stream supports float/double only\n";
  }
...
//
// NEW: exported stream-aware decompression entry point.
//
// This function is parallel to cuda_decompress(...), but calls decode_stream(...)
// so the actual cuZFP decompression kernel launches on cuda_stream.  The
// AllReduce hook can then enqueue the reduction kernel on the same stream
// without an extra synchronization between decompression and reduction.
//
void
cuda_decompress_stream(zfp_stream *stream,
                       zfp_field *field,
                       cudaStream_t cuda_stream)
{
  uint dims[3];
  ...
  {
    float *data = (float*) d_data;
    decoded_bytes = internal::decode_stream<float>(
        dims, stride, (int)stream->maxbits, d_stream, data, cuda_stream);
    d_data = (void*) data;
  }
  else if(field->type == zfp_type_double)
  {
    double *data = (double*) d_data;
    decoded_bytes = internal::decode_stream<double>(
        dims, stride, (int)stream->maxbits, d_stream, data, cuda_stream);
    d_data = (void*) data;
  }
  else
  {
    std::cerr << "stream-aware cuda_decompress_stream supports float/double only\n";
  }
